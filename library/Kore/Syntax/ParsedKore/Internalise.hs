...
computeTermIndex config =
    case lookForKCell config of
        Just (Def.SymbolApplication (Def.Application _ _ children)) ->
  ...
  where
    ...
    getTermIndex term =
        case term of
            Def.SymbolApplication (Def.Application symbol _ _) ->
    ...
    lookForKCell = para $ \case
        kCell@(Def.SymbolApplicationF (Def.Application symbol _ (children :: [(Def.Term, Maybe Def.Term)])))
    ...
    lookForTopTerm =
        \case
            Def.SymbolApplication (Def.Application symbol _ children)
                | symbol.name == "kseq" ->
    ...
    stripAwaySortInjections =
        \case
            term@(Def.SymbolApplication (Def.Application symbol _ children)) ->
