...
updateFrameFilter FrameFilter
    { matchingIds
    , notMatchingIds
    , notMatchingChildrenIds
    } matching notMatching notMatchingChildren frameId FrameName{unFrameName} =
    FrameFilter
  ...
  where
    match toMatch = if any (`Text.isInfixOf` unFrameName) toMatch
