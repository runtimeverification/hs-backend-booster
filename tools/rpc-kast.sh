...
run_pretty_json_request() {
    kore_json="$(jq .params.state.term $1)"
    if [[ "$kore_json" == "null" ]]; then
        echo "Error: malformed Kore JSON request"
        exit 1
    else
...
run_pretty_json_simplify_implication_response() {
    kore_json="$(jq .result.substitution.term $1)"
    if [[ "$kore_json" == "null" ]]; then
        echo "Error: malformed Kore JSON 'simplify-implication' response"
        exit 1
    else
        echo $kore_json | pyk json-to-kore | kast --definition $K_DEFINITION --input kore --output pretty /dev/stdin
    fi
...
if [[ "$run_command" == 'help' ]] || [[ "$run_command" == '--help' ]] ; then
    echo "
        ...
        usage: rpc-kast --request              : pretty print a kore-rpc request
               ...
               rpc-kast --simplify-implication : pretty print a kore-rpc 'simplify' response
    "
    exit 0
fi
...
case "$run_command" in
    ...
    "--simplify-implication" ) run_pretty_json_simplify_implication_response "$2"      ;;
