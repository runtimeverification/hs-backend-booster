...
export LLVM_LIB="$BUG_REPORT_DIR/interpreter.dylib"
...
do 
  echo "######## $tar ########"; 
  nix_shell "$SCRIPT_DIR/run-with-tarball.sh $tar -l Aborts --print-stats 2>&1 | tee $BUG_REPORT_DIR/$(basename $tar).out"; 
done
